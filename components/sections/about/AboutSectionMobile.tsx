...
import { AboutHeadlineWave } from "@/components/sections/about/about-section-shared";
import { AboutSectionCardsMobile } from "@/components/sections/about/AboutSectionCardsMobile";
import { cn } from "@/lib/utils";

type AboutSectionMobileProps = {
  className?: string;
};

export function AboutSectionMobile({ className }: AboutSectionMobileProps) {
      ...
      <div className="relative isolate flex flex-col gap-2 sm:gap-4 lg:gap-20">
        <div className="relative z-0 w-full px-1 sm:px-2">
          <h2 className="w-full text-left text-brand-black">
            <AboutHeadlineWave
              ...
              className={cn("block w-full", aboutTextSizes.title.mobile)}
            />
          </h2>
        </div>
        ...
        <div className={cn("relative z-10", "mt-0 px-1 sm:-mt-20 sm:px-2 lg:-mt-28")}>
          <AboutSectionCardsMobile
            titleTextSizeClassName={aboutTextSizes.cardTitle.mobile}
            bodyTextSizeClassName={aboutTextSizes.cardDescription.mobile}
          />
        </div>
      </div>
    </section>
