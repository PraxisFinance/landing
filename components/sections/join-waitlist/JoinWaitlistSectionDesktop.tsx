...
import { NEXT_IMAGE_SIZES_JOIN_WAITLIST_DESKTOP } from "@/components/constants/next-image-sizes";
import {
...
import type { JoinWaitlistCardRevealBag } from "@/components/sections/join-waitlist/use-join-waitlist-card-reveal";
import type { JoinWaitlistFormBag } from "@/components/sections/join-waitlist/use-join-waitlist-form";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { cn } from "@/lib/utils";

type JoinWaitlistSectionDesktopProps = JoinWaitlistFormBag &
  JoinWaitlistCardRevealBag & {
    className?: string;
  };

export function JoinWaitlistSectionDesktop({
  className,
  cardRef,
  shouldAnimateIn,
  walletAddress,
  ...
  submitError,
  onSubmitForm,
}: JoinWaitlistSectionDesktopProps) {
      ...
      >
        <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
          <div className="absolute inset-0 flex items-center justify-end">
            <div className="grid shrink-0 grid-cols-5 grid-rows-2 gap-4 sm:gap-5">
              {Array.from({ length: JOIN_WAITLIST_BG_CIRCLE_COUNT_DESKTOP }, (_, i) => (
                <div
                  key={i}
                  className="shrink-0 rounded-full bg-white opacity-25 sm:opacity-30"
                  style={{ width: JOIN_WAITLIST_BG_CIRCLE_PX_DESKTOP, height: JOIN_WAITLIST_BG_CIRCLE_PX_DESKTOP }}
                />
              ))}
            </div>
          </div>
        ...
        >
          <div className="flex min-w-0 flex-col text-center md:max-w-xl md:flex-1 md:py-1 md:text-left">
            <h2
              className={cn("text-balance text-brand-black", joinWaitlistTextSizes.title.desktop)}
            >
              ...
              {isAddressAccepted ? (
                ...
                <form className="flex flex-col gap-2" onSubmit={onSubmitForm}>
                  ...
                  <Button
                    ...
                    className={cn(
                      "mt-0 inline-flex h-11 w-auto gap-2 self-start rounded-lg px-7 text-base font-semibold shadow-none",
            ...
            <div className="mt-auto flex items-end gap-2 pt-5 md:pt-6">
        ...
        <div className="pointer-events-none absolute inset-y-0 right-0 z-[5] flex w-[54%] items-end justify-start">
          <Image
            ...
            width={560}
            height={560}
            sizes={NEXT_IMAGE_SIZES_JOIN_WAITLIST_DESKTOP}
