...
type JoinWaitlistSectionMobileProps = JoinWaitlistFormBag &
  JoinWaitlistCardRevealBag & {
    className?: string;
  };

export function JoinWaitlistSectionMobile({
  className,
  cardRef,
  shouldAnimateIn,
  walletAddress,
  ...
  handleJoinWaitlist,
  onSubmitForm,
}: JoinWaitlistSectionMobileProps) {
      ...
      >
        <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="grid shrink-0 grid-cols-3 grid-rows-2 gap-4 sm:gap-5">
              {Array.from({ length: JOIN_WAITLIST_BG_CIRCLE_COUNT_MOBILE }, (_, i) => (
                <div
                  key={i}
                  className="shrink-0 rounded-full bg-white opacity-25 sm:opacity-30"
                  style={{ width: JOIN_WAITLIST_BG_CIRCLE_PX_MOBILE, height: JOIN_WAITLIST_BG_CIRCLE_PX_MOBILE }}
                />
              ))}
            </div>
          </div>
        ...
        >
          <div className="flex min-w-0 flex-col text-center md:max-w-xl md:flex-1 md:py-1 md:text-left">
            <h2
              className={cn("text-balance text-brand-black", joinWaitlistTextSizes.title.mobile)}
            >
